 ...
 *         description: 스트릭 상태
 */
gamificationRoutes.get('/streak', requireDbUser, async (req, res) => {
  try {
 ...
 */
gamificationRoutes.post('/streak/check', requireDbUser, async (req, res) => {
  try {
    const userId = req.user!.dbUser!.id;
    const status = await streakService.checkStreakStatus(userId);
    ...
    res.status(500).json({ error: 'Failed to check streak status' });
  }
});
 ...
 */
gamificationRoutes.post('/streak/update', requireDbUser, async (req, res) => {
  try {
    const userId = req.user!.dbUser!.id;
