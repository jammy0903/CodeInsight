...
const C_CHAPTER_ZH: Record<string, LocalizedText> = {
  'c-0': { title: '控制流与函数', description: '循环与函数的基础语法' },
  'c-1': { title: '变量与内存', description: '理解变量如何占用内存' },
  'c-2': { title: '指针入门', description: '理解存储内存地址的变量' },
  'c-3': { title: '数组与指针', description: '连续内存空间与指针的紧密关系' },
  'c-4': { title: '函数与内存模型', description: '值传递、栈帧与指针参数' },
  'c-5': { title: '动态内存分配', description: '使用堆进行手动内存管理' },
  'c-6': { title: '结构体与字符串', description: '理解复合类型的内存布局' },
  'c-7': { title: '高级指针概念', description: '二级指针与复杂数据结构' },
  'c-8': { title: '函数指针与回调', description: '像传递数据一样传递代码' },
  'c-9': { title: '预处理器与宏', description: '编译前的代码生成与优化' },
  'c-10': { title: '文件 I/O 与系统资源', description: '文件输入输出：与操作系统通信' },
  'c-11': { title: '条件判断与分支', description: '使用 if-else、switch-case 与三元运算符控制流程' },
  'c-12': { title: '递归函数', description: '自调用函数原理与栈帧变化' },
  'c-13': { title: '数据类型与类型转换', description: 'C 语言中的多种数据类型与隐式/显式类型转换原理' },
};
