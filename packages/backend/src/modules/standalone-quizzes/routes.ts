...
export const standaloneQuizzesRoutes: FastifyPluginAsync = async (fastify) => {
  // =============================================
   ...
   * }
   */
  fastify.get('/chapters', { preHandler: [fastify.requireDbUser] }, async (request, reply) => {
    try {
      const userId = request.user!.dbUser!.id;
      const query = request.query as { language?: string; quizType?: string };
      const language = query.language;
      const quizType = query.quizType;
   ...
   * }
   */
  fastify.post('/attempt', { preHandler: [fastify.requireDbUser] }, async (request, reply) => {
    try {
      const userId = request.user!.dbUser!.id;
   ...
   * }
   */
  fastify.get('/weak-concepts', { preHandler: [fastify.requireDbUser] }, async (request, reply) => {
    try {
      const userId = request.user!.dbUser!.id;
      const query = request.query as { language?: string; limit?: string };
      const language = query.language;
