...
## 🚀 핵심 성능 전략: 하이브리드 콘텐츠 로딩 (DB + JSON)

C-OSINE은 레슨 콘텐츠의 빠른 로딩과 버전 관리를 위해 다음과 같은 하이브리드 전략을 사용합니다.

1.  **DB (메타데이터)**: 레슨 제목, 설명, 난이도 등 구조적 정보 저장 (Prisma 사용)
2.  **JSON (콘텐츠)**: 에디터 코드, 시뮬레이션 단계(steps), 퀴즈 등 무거운 데이터는 JSON 파일로 저장
3.  **인메모리 캐싱**: 서버 시작 시 `LessonContentLoader`가 모든 JSON을 메모리에 로드 (O(1) 조회)

**효과**:
- DB 호출 대비 **10-100배 빠른 응답 속도** (0.1ms 내외)
- 콘텐츠 변경 시 Git으로 버전 관리 및 복구 용이
