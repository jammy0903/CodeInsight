...
### 5. 실행 흐름 끝까지 추적 후 답변 ⭐
- 파일 존재/삭제 여부만으로 "쓴다/안 쓴다" 판단 **절대 금지**
- 진입점(Page) → 렌더링 경로 → 데이터 도착 여부까지 코드를 **끝까지 따라간 후**에만 사실로 답변
- 확인 안 된 건 **"확인 안 됨"**이라고 명시, 추측을 사실처럼 말하지 않기
- 예: "JavaFlowView 삭제됨 → Flow 안 씀" ❌ → 범용 파이프라인이 대신 렌더링할 수 있으므로 실제 렌더링 경로 확인 필수
